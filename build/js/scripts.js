...
function activeStart() {
    if (firstPageInput.value.length > 0) {
        ...
        firstPageButtonStart.style.color = 'black';
        firstPageButtonStart.disabled = true;
    }
}
...
function activePage(event) {
    ...
    if (firstPageInput.value === "") {
        firstPageButtonStart.disabled = true;
        alert("Сперва введите свое имя!");
        return;
    } else {
...
function activeTwoPageAndHideTreePage() {
  ...
  consoleDiv.innerHTML = '<h1 class="makeAChoise"> Make a choice! </h1>';
  healthRefresh();
  form.reset();
